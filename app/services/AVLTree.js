...
// Midified by Pernes Panta.
// Pass in the attribute you want to use for comparing

var avlTreeFactory = angular.module('AVLTree', []);

avlTreeFactory.factory('AVLTree', function(){
    ...
    AVLTree.prototype.toString = function(){
        var s = this.node[this.attr] + '; ';
        if(this.left != null){
            s += ''+ this.node[this.attr] + ' -> ' +this.left.toString();
            this.node.nodeInfo.sum += this.left.node.nodeInfo.sum;
        }
        if(this.right != null){
            s += '' + this.node[this.attr] + ' -> ' + this.right.toString();
             this.node.nodeInfo.sum += this.right.node.nodeInfo.sum;
        }
        
        return s;
    }

    AVLTree.build = function(data){
        return new AVLTree(
            data.node,
            data.attr
        );
